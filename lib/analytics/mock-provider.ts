...
export const mockAnalyticsProvider: AnalyticsProvider = {
  ...
  async getHeatmap(_siteId, _range, filters) {
    return { pagePath: filters.pagePath ?? "/", device: (filters.device ?? "mobile") as DeviceType, sampleSize: 2184, pageHeight: 5200, points, attentionBands: Array.from({ length: 20 }, (_, index) => ({ index, seconds: Math.max(12, 180 - index * 7 + (index % 4) * 24), weight: Math.max(.12, 1 - index * .035 + (index % 4) * .1) })), scrollReach: [{ depth: 25, percentage: 91 }, { depth: 50, percentage: 68 }, { depth: 75, percentage: 42 }, { depth: 90, percentage: 21 }] };
  },
