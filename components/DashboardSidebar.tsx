...
export function DashboardSidebar() {
  ...
  return <aside className="sidebar"><div className="logo"><Image src="/ismo-symbol.png" width={34} height={34} alt="" priority /><div><b>ismo<span className="brand-dot">.</span></b><small>WEB ANALYTICS</small></div></div><SiteSwitcher /><nav>{navGroups.map(group => <div className="nav-group" key={group}><span>{group}</span>{nav.filter(item => item.group === group).map(({ label, icon: Icon }) => <Link key={label} className={pathname === screenRoutes[label] ? "active" : ""} href={screenRoutes[label]}><Icon size={19} weight={pathname === screenRoutes[label] ? "fill" : "regular"} />{label}</Link>)}</div>)}</nav><div className="sidebar-bottom"><Link className={pathname === "/settings" ? "active" : ""} href="/settings"><Gear />サイト設定</Link><div className="profile"><div>MK</div><span><b>MOGCIA Inc.</b><small>Admin</small></span><CaretDown /></div></div></aside>;
}
