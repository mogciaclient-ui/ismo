...
export function SettingsScreen() {
  ...
  return <>
    ...
    {tab === "site" && <section className="settings-panel"><h2>サイト情報</h2><p>分析対象と計測時の基本ルールを設定します。</p><div className="form-grid"><label><span>表示名</span><input value={settings.name} onChange={e => update({ name: e.target.value })}/></label><label><span>ドメイン</span><div className="input-prefix"><i>https://</i><input value={settings.domain} onChange={e => update({ domain: e.target.value })}/></div></label><label><span>タイムゾーン</span><select value={settings.timezone} onChange={e => update({ timezone: e.target.value })}><option>Asia/Tokyo</option><option>UTC</option></select></label><label><span>同意モード</span><select value={settings.consentMode} onChange={e => update({ consentMode: e.target.value as SiteSettings["consentMode"] })}><option value="required">同意後のみ計測</option><option value="analytics_only">常時計測</option></select></label><label><span>プライバシーポリシーURL</span><input type="url" value={settings.privacyUrl??""} placeholder="https://www.mogcia.net/privacy/" onChange={e => update({ privacyUrl: e.target.value })}/></label><label><span>データ保持期間</span><select value={settings.retentionDays??395} onChange={e => update({ retentionDays: Number(e.target.value) })}><option value={90}>90日</option><option value={395}>13か月</option><option value={730}>24か月</option></select></label></div><div className="privacy-box"><WarningCircle/><div><b>プライバシー設計</b><p>フォーム入力値、氏名、メールアドレスは収集しません。個人ではなく匿名セッション単位で分析します。期限を過ぎたイベントはFirestore TTLで自動削除します。</p></div></div></section>}
